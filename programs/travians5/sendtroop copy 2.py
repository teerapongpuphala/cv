...
timeSpendList = timeSpendString.split(":")
timeSpendHour = int(timeSpendList[0])
timeSpendMin = int(timeSpendList[1])
timeSpendSec = int(timeSpendList[2])




timeInput = input("Arrive time(inform tt-mm-ss) at : ")
timeInputList = timeInput.split("-")
timeInputHour = int(timeInputList[0])
timeInputMin = int(timeInputList[1])
timeInputSec = int(timeInputList[2])


# arrTimeString = form.find('span',class_="timer").string
# print(arrTimeString)
...
while(not end):
    ...
    if (currentHour+timeSpendHour == timeInputHour):
        currentMin = int(datetime.now().strftime("%M"))
        if (currentMin+timeSpendMin == timeInputMin):
            currentSec = int(datetime.now().strftime("%S"))
            if (currentSec+timeSpendSec == timeInputSec):
                confirmDicts={}
